    ...
    if request.method == "POST":
        ...
        for i in range(int(len(productos_factura) / 3)):
            ...
            agregarProductoFactura(
                cursor, producto_nuevo[0], folio, producto_nuevo[1], producto_nuevo[2]
            )
        print(
            fecha_emision,
            descripcion,
...
def cotizaciones():
    ...
    return render_template(
        ...
        productos=productos,
        maximo=maximo,
        id_cotizacion=id_cotizacion[0][0] + 1,
        fecha=fecha,
    )
...
def verfacturas():
    ...
    return render_template(
        "verfacturas.html", lista_facturas=facturas, maximo=maximo, clientes=nombres
    )
